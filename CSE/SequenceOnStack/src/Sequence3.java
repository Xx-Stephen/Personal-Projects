...
public class Sequence3<T> extends SequenceSecondary<T> {
    ...
    private static <T> void setLengthOfLeftStack(Stack<T> leftStack,
        ...
        Stack<T> temp = leftStack.newInstance();
        temp.transferFrom(leftStack);
        while (temp.length() != 0) {
            T element = temp.pop();
            leftStack.push(element);
        }
        if (newLeftLength > leftStack.length()) {
            newLeftLength = newLeftLength - leftStack.length();
            Stack<T> temp2 = rightStack.newInstance();
            Stack<T> temp3 = rightStack.newInstance();
            temp2.transferFrom(rightStack);
