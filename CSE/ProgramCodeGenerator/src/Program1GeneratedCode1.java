...
public final class Program1GeneratedCode1 extends Program1 {
    ...
    private static void generateCodeForStatement(Statement s,
        ...
        switch (s.kind()) {
            ...
            case IF_ELSE: {
                Statement b = s.newInstance();
                Statement d = s.newInstance();
                Condition c = s.disassembleIfElse(b, d);
                cp.add(cp.length(), conditionalJump(c).byteCode());
                int jump = cp.length();
                cp.add(cp.length(), dummy);
                generateCodeForStatement(d, context, cp);
                cp.add(cp.length(), Instruction.JUMP.byteCode());
                int jump3 = cp.length();
                cp.add(cp.length(), dummy);
                int jump4 = cp.length();
                generateCodeForStatement(b, context, cp);
                cp.replaceEntry(jump, jump4);
                cp.replaceEntry(jump3, cp.length());
                s.assembleIfElse(c, b, d);
            ...
            case CALL: {
                ...
                Instruction num = Instruction.valueOf(name);
                int res = num.byteCode();
                cp.add(cp.length(), res);
                s.assembleCall(name);
