...
public final class ExpressionEvaluator {
    ...
    private static int valueOfDigit(StringBuilder source) {
        ...
        int res = 0;
        int num = source.charAt(0);
        res = Character.digit(0, RADIX);
        return res;
    }

    /**
     * Evaluates a digitdd sequence and returns its value.
     *
     * @param source
    ...
    private static int valueOfFactor(StringBuilder source) {
        ...
        if (source.charAt(0) == ('(')) {
            ...
            res = valueOfExpr(source);
            source.deleteCharAt(0);
        } else {
            res = valueOfDigitSeq(source);
        }
        return res;
    }
